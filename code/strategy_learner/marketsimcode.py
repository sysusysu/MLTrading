...
def assess_portfolio(df_value, bench, names, df_order=None, sf=252.0, rfr=0.0, plot=False):
    ...
    return result
def assess_portfolio2(df_value,df_value2, bench, names,  sf=252.0, rfr=0.0, plot=False):
    merged = pd.concat([df_value,df_value2, bench], axis=1)
    merged.rename(columns = {0:names[0],1:names[1],2:names[2]}, inplace=True)
    merged = merged.div(merged.iloc[0])  ## normalize stock price to 1 when initializing
    # Get portfolio statistics (note: std_daily_ret = volatility)
    #cr, adr, sddr, sr = [0.25, 0.001, 0.0005, 2.1] # add code here to compute stats
    cr = merged.iloc[-1,:]/merged.iloc[0,:]-1
    ...
    ev = merged.iloc[-1,:]*100000
    tmp = []
    tmp.append(['cumulative return']+list(cr))
    tmp.append(['average daily return']+list(adr))
    tmp.append(['std daily return']+list(sddr))
    tmp.append(['sharpe ratio']+list(sr))
    tmp.append(['end value']+list(ev))
    
    result = pd.DataFrame(tmp, columns = ['Measurement']+names)
    
    if plot:
        plot_mult_col(merged, names, size = (10,5))
...
def plot_two_col(data, columns, df_order = None, size = (20,10), perc = 1):
    ...
    plt.show()
def plot_mult_col(data, columns, size = (20,10), perc = 1):
    data = data.sort_index()
    length = data.shape[0]
    start_idx = int((1-perc)*length)
    ...
    for i in range(len(columns)):
        y1 = data[columns[i]][start_idx:]
        p1=""
        if columns[1]=="Bench":
            p1, = ax1.plot(x,y1, color="black")
        else:
            p1, = ax1.plot(x,y1)
        p1.set_label(columns[i])
